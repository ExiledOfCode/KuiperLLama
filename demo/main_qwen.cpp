...
std::string escape_inline_text(const std::string& text, size_t limit = 48) {
  std::string output;
  output.reserve(std::min(text.size(), limit) + 8);
  for (char ch : text) {
    if (ch == '\n') {
      output += "\\n";
    } else if (ch == '\r') {
...
std::vector<std::string> build_token_pieces_preview(const model::Qwen2Model& model,
  ...
  for (size_t i = 0; i < count; ++i) {
    const std::string piece = escape_inline_text(model.decode(token_ids[i]), 64);
...
GenerationResult generate(const model::Qwen2Model& model, const std::string& sentence,
  ...
  while (pos < total_steps) {
    ...
    if (is_prompt) {
      ...
      if (generated_token_count <= kTraceSamplingPreviewLimit) {
        const std::string selected_token = escape_inline_text(model.decode(next), 64);
        emit_trace_json(
            json{{"event", "sampling"},
                 {"step", "sampling"},
  ...
  const std::vector<model::OpProfileStat> op_profile_stats = model.get_profile_stats();
...
int run_single_shot(const char* checkpoint_path, const char* tokenizer_path, const std::string& prompt,
  ...
  GenerationResult result = generate(model, prompt, max_steps, false);
...
int run_serve(const char* checkpoint_path, const char* tokenizer_path, int max_steps) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    const auto start = std::chrono::steady_clock::now();
    GenerationResult result = generate(model, prompt, max_steps, true);
    const auto end = std::chrono::steady_clock::now();
    const auto duration = std::chrono::duration<double>(end - start).count();
    emit_trace_json(
        json{{"event", "done"},
