...
size_t qwen2_rope_fill_progress_bytes(const model::TransformerConfig& config) {
...
size_t qwen2_runtime_progress_bytes(const model::TransformerConfig& config,
                                    int32_t kv_cache_tokens) {
  size_t total = 0;
  total += dims_byte_size(base::DataType::kDataTypeInt32, {1});
  total +=
      dims_byte_size(base::DataType::kDataTypeFp32, {1, static_cast<size_t>(config.dim_)});
  ...
  total += qwen2_rope_fill_progress_bytes(config);
  return total;
}
...
std::shared_ptr<base::Buffer> Qwen2Layers::to_cuda(std::shared_ptr<kernel::CudaConfig> config,
  ...
  if (optimized_weight_loading) {
    ...
    LOG(WARNING) << "Falling back to legacy per-layer CUDA weight upload for Qwen2.";
  }
...
base::Status Qwen2Model::init(base::DeviceType device_type) {
  ...
  if (device_type_ == base::DeviceType::kDeviceCPU) {
    ...
    const size_t runtime_total_bytes = qwen2_runtime_progress_bytes(*config_, kv_cache_tokens);
    const size_t rope_fill_bytes = qwen2_rope_fill_progress_bytes(*config_);
...
void Qwen2Model::create_param_layers() {
  ...
  qwen_layers_->embedding_layer_->set_weight(0, {std::abs(config_->vocab_size_), config_->dim_},
                                             weight_embedding, cpu_device_type, weight_data_type_);
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    ...
    wq->set_weight(0, {dim, dim}, this->raw_model_data_->weight(pos), cpu_device_type,
                   weight_data_type_);
    pos += dim * dim;
    wq->set_bias(0, dim, this->raw_model_data_->weight(pos), cpu_device_type, weight_data_type_);
    pos += dim;
    qwen_layers_->wq_layers_.push_back(wq);
  }
...
void Qwen2Model::init_mem() {
  ...
  if (device_type_ == base::DeviceType::kDeviceCUDA) {
    ...
    runtime_total_bytes = qwen2_runtime_progress_bytes(*config_, kv_cache_tokens);
