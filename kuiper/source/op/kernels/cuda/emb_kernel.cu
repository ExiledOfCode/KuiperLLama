...
__device__ inline float emb_to_float(WeightT value);

template <>
__device__ inline float emb_to_float<float>(float value) {
  return value;
}

template <>
__device__ inline float emb_to_float<__nv_bfloat16>(__nv_bfloat16 value) {
...
template <typename WeightT>
__global__ void emb_kernel_cu_mixed(int32_t vocab_size, int32_t token_num, int32_t weight_dim,
                                    const int32_t* input_ptr, const WeightT* weight_ptr,
                                    float* output_ptr) {
  int32_t token_idx = blockIdx.x;
  if (token_idx >= token_num) {
    return;
  ...
  float* output_ptr_start = output_ptr + token_idx * weight_dim;
  const WeightT* weight_ptr_start = weight_ptr + token * weight_dim;

  for (int32_t i = threadIdx.x; i < weight_dim; i += blockDim.x) {
    output_ptr_start[i] = emb_to_float(weight_ptr_start[i]);
  }
}
...
void emb_kernel_cu(const tensor::Tensor& input, const tensor::Tensor& weight,
  ...
  if (weight.data_type() == base::DataType::kDataTypeBf16) {
    ...
    if (stream) {
      ...
      emb_kernel_cu_mixed<<<max_seq_len, thread_num, 0, stream_>>>(vocab_size, input_num,
                                                                   weight_dim, in_ptr, wei_ptr,
                                                                   out_ptr);
    } else {
      emb_kernel_cu_mixed<<<max_seq_len, thread_num>>>(vocab_size, input_num, weight_dim, in_ptr,
                                                       wei_ptr, out_ptr);
    }
  } else {
    ...
    if (stream) {
      cudaStream_t stream_ = static_cast<cudaStream_t>(stream);
      emb_kernel_cu_fp32<<<max_seq_len, thread_num, 0, stream_>>>(vocab_size, input_num,
                                                                  weight_dim, in_ptr, wei_ptr,
                                                                  out_ptr);
    } else {
